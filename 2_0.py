...
def show_info():
    info_message = f"   Expense Calculator Beta{current_version}\n\n   @NeonWaa\n"
...
def apply_dark_mode_theme():
    window.configure(bg="black")  # Set root window background to black
    style.set_theme("black")  # Apply the dark mode theme
...
center_frame.pack(fill="both", expand=True, anchor="center")

# Create a canvas with scrolling capabilities
canvas = tk.Canvas(center_frame)
canvas.pack(side="left", fill="both", expand=True)

# Create a new frame inside the canvas to contain the widgets
...
def center_inner_frame(event):
    ...
    canvas.coords(inner_frame_id, (canvas_width - inner_frame_width) // 2, 0)
...
canvas.bind("<Configure>", center_inner_frame)
...
center_frame.pack(fill="both", expand=True)  # Updated line
